...
const createReservation = async () => {
  if (!guestName.value || !guestEmail.value || selectedDates.value.length === 0) {
    error.value = 'Please fill all fields and select dates'
    return
  }
    ...
    <div v-else-if="room" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div>
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
          <div class="h-64 bg-gradient-to-br from-primary-400 to-primary-600"></div>
          <div class="p-6">
            <h1 class="text-3xl font-bold text-secondary-900 mb-2">
              {{ room.name }}
            </h1>
            <p class="text-secondary-600">Room ID: {{ room.id }}</p>
          </div>
        </div>
